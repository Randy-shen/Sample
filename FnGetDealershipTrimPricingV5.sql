CREATE FUNCTION [dlr].[FnGetDealershipTrimPricingV5]
(	
				...
				FROM
					(       
						SELECT
							...
							IsClosestFlag,
							IsDisplayedFlag = 
								CASE WHEN (@BestPriceQuote = 1 AND SortOrder <= 1) or (@BestPriceQuote = 0 and (SortOrder <= 3 OR IsTexasZoneFlag = 1 OR DealershipId IS null)) THEN 1 ELSE 0 END,
							IsLowestFlag,
							SortOrder,
							AvailableDealershipCount,
							DsaTotal,
									...
									( 
										...
										FROM
											(  
												...
												LEFT OUTER JOIN 
													dlr.FnGetDealershipsV2(@ProgramAffinityGroupId, @SearchPostalCode, CASE WHEN @QueryType IN ('All', 'Comparables') THEN NULL ELSE @MakeId end, @distance, @DefaultDealershipId, @AbTests, @IncludeAllDealershipsForLogging) D ON T.MakeId = D.MakeId
												OUTER APPLY                                                     
						...
						OR 
							CASE WHEN (@BestPriceQuote = 1 AND SortOrder <= 1) or (@BestPriceQuote = 0 and (SortOrder <= 3 OR IsTexasZoneFlag = 1 OR DealershipId IS null)) THEN 1 ELSE 0 END = 1 -- IsDisplayedFlag
					) X  
				OUTER APPLY
